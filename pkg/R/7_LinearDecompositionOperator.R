...
setClass(# a decomposition operator described by a matrix valued function of time
    Class="LinearDecompositionOperator",
...
setMethod(
    f="initialize",
    ### 
    signature="LinearDecompositionOperator",
...
setMethod(
      ...
      ### This method creates a LinearDecompositionOperator from a timedependent function and a lag only
      (map,lag=0){
      return(new("LinearDecompositionOperator",starttime=-Inf,endtime=Inf,map=map))
    }
)
#---------------------------------------------------------------------
setMethod(
