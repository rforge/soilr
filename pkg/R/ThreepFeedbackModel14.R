...
{ 
  ...
  ex=function(){
    ...
    Feedback=ThreepFeedbackModel14(t=years,ks=c(k1=k1, k2=k2, k3=k3),C0=c(100,500,1000), F0_Delta14C=c(0,0,0),In=LitterInput, a21=0.2*k1,a12=0.2*k2,a32=0.1*k2,a23=0.2*k3,FcAtm=C14Atm_NH)
    F.R14m=getF14R(Feedback)
    F.C14m=getF14C(Feedback)
    F.C14t=getF14(Feedback)
    
    Series=ThreepSeriesModel14(t=years,ks=c(k1=1/2, k2=1/35, k3=1/100),C0=c(100,500,1000), F0_Delta14C=c(0,0,0),In=LitterInput, a21=0.2*k1, a32=0.1*k2,FcAtm=C14Atm_NH)
    S.R14m=getF14R(Series)
    S.C14m=getF14C(Series)
    S.C14t=getF14(Series)
    
    Parallel=ThreepParallelModel14(t=years,ks=c(k1=1/2, k2=1/35, k3=1/100),C0=c(100,500,1000), F0_Delta14C=c(0,0,0),In=LitterInput, gam1=0.6, gam2=0.2, FcAtm=C14Atm_NH,lag=2)
    ...
    legend("topright",c("Atmosphere", "Pool 1", "Pool 2", "Pool 3"),lty=rep(1,4),col=c(1,4,4,4),lwd=c(1,1,2,3),bty="n")
    
    plot(C14Atm_NH,type="l",xlab="Year",ylab=expression(paste(Delta^14,"C ","(\u2030)")),xlim=c(1940,2010)) 
    lines(years,P.C14m,col=4)
    lines(years,P.R14m,col=2)
    legend("topright",c("Atmosphere","Bulk SOM", "Respired C"),lty=c(1,1,1), col=c(1,4,2),bty="n")
