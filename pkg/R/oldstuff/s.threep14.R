s.threep14 <-
function(C10, C20, C30, In, k1, k2, k3, rf1, rf2){
        ...
        for(i in 2:length(yr)){
           ...
           F2t[i]=(F2t[i-1]*C2t[i-1]*(1-k2[i-1]-lambda)+(F1t[i-1]*C1t[i-1]*k1[i-1]*(1-rf1)))/C2t[i]
           F3t[i]=(F3t[i-1]*C3t[i-1]*(1-k3[i-1]-lambda)+(F2t[i-1]*C2t[i-1]*k2[i-1]*(1-rf2)))/C3t[i]
           
           R1Ft[i]=F1t[i]*R1t[i]
           R2Ft[i]=F2t[i]*R2t[i]
           R3Ft[i]=F3t[i]*R3t[i]
           Ft[i]=(F1t[i]*C1t[i] + F2t[i]*C2t[i] + F3t[i]*C3t[i])/(C1t[i]+C2t[i]+C3t[i])
           RFt[i]=(R1Ft[i] + R2Ft[i] + R3Ft[i])/(R1t[i]+R2t[i]+R3t[i])
           
           }
        
    return(list(year=yr,Ct=C1t+C2t+C3t,Rt=R1t+R2t+R3t,F1t=F1t, F2t=F2t, F3t=F3t,
                Ft=Ft, Del14C=(Ft-1)*1000, RFt=RFt, R14C=(RFt-1)*1000,C1t=C1t,C2t=C2t,C3t=C3t,
                R1t=R1t,R2t=R2t,R3t=R3t,Del14C1=(F1t-1)*1000,Del14C2=(F2t-1)*1000,Del14C3=(F3t-1)*1000,
                R14C1=((R1Ft/R1t)-1)*1000,R14C2=((R2Ft/R2t)-1)*1000,R14C3=((R3Ft/R3t)-1)*1000))
}
