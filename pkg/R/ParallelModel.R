...
(times,		##<< A vector containing the points in time where the solution is sought.
 coeffs,	##<< A vector valued function containing the decay rates for the n pools as function of time. The length of the vector is equal to the number of pools.
 ...
 solverfunc =deSolve.lsoda.wrapper    ##<< The function used to actually solve the ODE system. This can be \code{\link{SoilR.euler}} or \code{\link{deSolve.lsoda.wrapper}} or any other user provided function with the same interface. 
    ...
    obj=new(Class="Model",times,A,startvalues,inputrates,solverfunc)
### a model object
}
,ex=function(){
      t_start=0 
      ...
      k=function(times){c(0.5,0.2,0.3)}
      c0=c(1, 2, 3)
      #constant inputrates
      inputrates=function(t){matrix(nrow=3,ncol=1,c(1,1,1))}
