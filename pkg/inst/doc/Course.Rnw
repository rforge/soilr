...
The reason is this:
If you create a model in \SoilR \, the package will by default execute an number of tests on the given arguments to prevent the unintentional creation of models that are not biologically meaningful.
