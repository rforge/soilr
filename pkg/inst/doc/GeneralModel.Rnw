...
\author{Carlos Sierra, Susan Trumbore, Markus Müller }
...
# 2.)
# Note that we use a smaller time range for the 
# inputFlux dataframe than in the previous part 
# of the document
#
# first we create a helper for the paths
sp=function(fn){file.path("..","..","data",fn)}
# now we create a dataframe using the same time 
# rangeas all the examples before 
t_peak1=0.75
t_peak2=1.75
df=data.frame("time"=t,"inputFlux"=exp(-((t-t_peak1)*40.0)^2)+exp(-((t-t_peak2)*40.0)^2))
write.csv(df,p)

# Now we create a second dataset spanning a smaller 
# time range which we will use later to demonstrate 
# the safety net provided by the use of TimeMap 
d=t_end-t_start
ts2=t_start+d/4
te2=t_end-d/4
df2=data.frame("time"=t,"inputFlux"=exp(-((t-t_peak1)*40.0)^2)+exp(-((t-t_peak2)*40.0)^2))
write.csv(df2,p2)
#write.csv(df,sp(fn)) uncommente this to update the dataset
#write.csv(df2,sp(fn2))
@
...
iTm=TimeMap.from.Dataframe(df)
@
If we try to create a Model from this we will get
